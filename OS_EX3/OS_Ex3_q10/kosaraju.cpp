...
void* waiting_thread(void* arg)
{
    ...
    while(!cond_exit){
        printf("waiting_thread: Waiting for condition...\n");
        while (!condition && !cond_exit)
        {
        pthread_cond_wait(&cond, &mutex);
        printf("waiting_thread: Woke up from condition wait\n");
        }
            // Handle the condition based on SCC_Graph or any other logic
            if (!SCC_Graph) {
...
int main() {
    ...
    if (server_fd < 0) {
        ...
        return 1;
         ...
         pthread_create(&sleep_thread, NULL, waiting_thread, NULL);
